...
async function resolveVideo(browser) {
  ...
  try {
    ...
    const nav = await page.goto(`https://www.youtube.com/watch?v=${TEST_VIDEO_ID}`, {
    ...
    return { nav_status: nav?.status() ?? null, player };
  } finally {
    await page.close().catch(() => {});
  }
}

async function readFixedRange(browser, format) {
  const sourceTotal = Number(format?.contentLength || 0);
  const start = RANGE_START;
  const end = sourceTotal
    ? Math.min(start + RANGE_LENGTH - 1, sourceTotal - 1)
  ...
  try {
    ...
    if (received !== expected) {
      throw new Error(`CDP stream size mismatch: expected ${expected}, got ${received}`);
    }
...
async function deliverChunk(env) {
  const browser = await puppeteer.launch(env.BROWSER);
  try {
    const resolved = await resolveVideo(browser);
    ...
    const chunk = await readFixedRange(browser, player.video);
    return new Response(chunk.body, {
      status: 200,
      headers: {
        "Content-Type": chunk.contentType,
        "Content-Length": String(chunk.received),
        "Cache-Control": "no-store",
        ...
        "X-CDP-Read-Count": String(chunk.readCount),
        "X-Browser-Run": "true",
  ...
  async fetch(request, env) {
    ...
    try {
      if (url.pathname === "/chunk/video") {
        return await deliverChunk(env);
      }
      return json({
        service: "youtube-free-browser-visionos-binary-probe",

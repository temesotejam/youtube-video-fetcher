...
const FIXED_OFFSETS = [0, 4 * 1024 * 1024, 20 * 1024 * 1024, 40 * 1024 * 1024, 60 * 1024 * 1024];
const ANDROID_VR = {
  id: 28,
...
async function runProbe(env, videoId) {
  ...
  try {
    ...
    const initialPlayer = await getAndroidVrVideo(resolver.page, videoId);
    if (initialPlayer.playability_status !== "OK" || !initialPlayer.video?.url) {
      throw new Error(
        `Player not OK: ${initialPlayer.playability_status || "unknown"} ${initialPlayer.playability_reason || initialPlayer.error || ""}`,
      );
    }

    const sourceTotalBytes = Number(initialPlayer.video.contentLength || 0);
    const tailStart = Math.max(0, sourceTotalBytes - CHUNK_BYTES);
    const offsets = [...FIXED_OFFSETS, tailStart]
      .filter((value) => value < sourceTotalBytes)
    ...
    for (const start of offsets) {
      try {
        ...
        const range = await readRange(browser, player.video, start, CHUNK_BYTES);
        results.push({
          ...range,
    ...
    return {
      result: "single-browser-cdp-offset-probe-complete",
      ...
      offsets,
      results,
      all_success: results.length === offsets.length && results.every((item) => item.success),
...
export default {
  async fetch(request, env) {
    ...
    try {
      if (url.pathname === "/cdp/probe/offsets") {
        return json(await runProbe(env, videoId));
      }
      return json({
        service: "youtube-free-browser-cdp-offset-probe",
        endpoints: ["/cdp/probe/offsets"],
